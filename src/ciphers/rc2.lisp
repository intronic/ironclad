...
(macrolet ((mix (index)
             (loop for i from 0 below 4
                   collect (let ((x0 (case-intern (format nil "r~D" i)))
                                 (x1 (case-intern (format nil "r~D" (mod (- i 1) 4))))
                                 (x2 (case-intern (format nil "r~D" (mod (- i 2) 4))))
                                 (x3 (case-intern (format nil "r~D" (mod (- i 3) 4)))))
                             `(progn
           ...
           (rmash ()
             (loop for i from 0 below 4
                   collect (let ((x0 (case-intern (format nil "r~D" (mod i 4))))
                                 (x1 (case-intern (format nil "r~D" (mod (- i 1) 4)))))
                             `(setf ,x0 (ldb (byte 16 0)
                                         (- ,x0 (aref round-keys (ldb (byte 6 0) ,x1)))))) into forms
                   finally (return `(progn ,@(nreverse forms))))))
