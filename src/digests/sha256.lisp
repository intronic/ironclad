    ...
    (flet ((ch (x y z)
      ...
      (macrolet ((sha256-round (i a b c d e f g h)
        ...
        #.(loop for slot in '(a b c d e f g h)
                collect (let ((regs-accessor (case-intern (format nil "sha256-regs-~A" slot))))
  ...
  (let ((regs (sha256-regs state))
	...
	(setq start (the fixnum (+ start amount)))
        (let ((new-index (mod (+ buffer-index amount) 64)))
          ...
          (when (>= start end)
            (setf (sha256-buffer-index state) new-index)
            (incf (sha256-amount state) length)
    ...
    (loop for offset of-type index from start below end by 64
          ...
          (let ((amount (- end offset)))
            ...
            (setf (sha256-buffer-index state) amount)))
    (incf (sha256-amount state) length)
    state))
